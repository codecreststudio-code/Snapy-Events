    ...
    <Link href={`/blog/${post.slug}`} className="group flex flex-col rounded-2xl border border-hairline-dark bg-surface-card overflow-hidden hover:border-mauve/40 transition-all duration-300">
      <div className="aspect-video overflow-hidden bg-ink/5">
        {post.cover_image_url ? (
          ...
          <div className="w-full h-full flex items-center justify-center"><BookOpen className="h-8 w-8 text-ink-tertiary" /></div>
        )}
      </div>
      ...
      <main className="flex-1">
        <section className="border-b border-hairline-dark bg-surface-card py-14 md:py-20">
          <div className="container mx-auto max-w-3xl px-6 text-center">
            ...
            <p className="text-ink-secondary font-light">{posts.length} articles tagged with #{t.name}</p>
          </div>
        </section>
        ...
        <section className="container mx-auto max-w-5xl px-6 py-12">
          {posts.length > 0 ? (
            ...
            <div className="py-24 text-center">
              ...
              <p className="text-ink-secondary">No articles with this tag yet.</p>
