...
function formatContent(content: string) {
  const rendered = stripDangerousHtml(content)
    // h2
    .replace(/^## (.+)$/gm, '<h2 class="text-2xl font-bold text-ink mt-10 mb-4 tracking-tight">$1</h2>')
    // h3
    .replace(/^### (.+)$/gm, '<h3 class="text-lg font-bold text-ink mt-7 mb-3">$1</h3>')
    // h4
    .replace(/^#### (.+)$/gm, '<h4 class="text-base font-semibold text-ink mt-5 mb-2">$1</h4>')
    // bold
    .replace(/\*\*(.+?)\*\*/g, '<strong class="font-semibold text-ink">$1</strong>')
    // italic
    .replace(/\*(.+?)\*/g, '<em class="italic">$1</em>')
    // inline code
    .replace(/`([^`]+)`/g, '<code class="font-mono text-sm bg-mauve/10 text-mauve px-1.5 py-0.5 rounded">$1</code>')
    // blockquote
    .replace(/^> (.+)$/gm, '<blockquote class="border-l-4 border-mauve/40 pl-5 py-2 my-5 text-ink-secondary italic bg-mauve/10 rounded-r-xl">$1</blockquote>')
    // hr
    ...
    .replace(/^\|(.+)\|$/gm, (match: string, row: string) => {
      ...
      if (isHeader) return ""
      const tag = "td"
      return `<tr>${cells.map((c: string) => `<${tag} class="border border-hairline-dark px-4 py-2 text-sm text-ink-secondary">${c}</${tag}>`).join("")}</tr>`
    })
    // wrap tables
    ...
    .replace(/^(?!<)(.+)$/gm, '<p class="text-ink-secondary text-sm leading-relaxed my-3">$1</p>')
    // checkmarks ✅
...
function ShareButtons({ title, slug }: { title: string; slug: string }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <span className="text-xs text-ink-secondary mr-1">Share:</span>
      <a
        href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(title)}&url=${encodeURIComponent(url)}`}
        target="_blank"
        rel="noopener noreferrer"
        className="flex h-8 w-8 items-center justify-center rounded-full bg-mauve/5 text-ink-secondary hover:bg-mauve/10 hover:text-ink transition-colors"
      ...
      <button
        onClick={handleCopy}
        className="flex h-8 w-8 items-center justify-center rounded-full bg-mauve/5 text-ink-secondary hover:bg-mauve/15 hover:text-mauve transition-colors"
      >
  ...
  if (done) {
    return (
      <div className="flex items-center gap-3 py-2">
        ...
        <p className="text-sm text-ink font-medium">You&apos;re subscribed! Great tips coming your way.</p>
      </div>
    )
  }
...
function RelatedCard({ post }: { post: BlogPost }) {
  return (
    <Link href={`/blog/${post.slug}`} className="group flex gap-4 items-start py-3 border-b border-hairline-dark last:border-0">
      <div className="h-14 w-20 rounded-xl overflow-hidden bg-ink/5 shrink-0">
      ...
      <div className="flex-1 min-w-0">
        <p className="text-xs font-semibold text-ink leading-snug line-clamp-2 group-hover:text-mauve transition-colors">{post.title}</p>
...
export default function BlogPostClient({ post, related }: { post: BlogPost; related: BlogPost[] }) {
  ...
  return (
    <div className={`flex min-h-screen flex-col bg-surface-dark text-ink ${inter.className}`}>
      ...
      <div className="fixed top-0 left-0 right-0 h-0.5 z-50 bg-ink/10">
        <motion.div
        ...
        <div className="container mx-auto max-w-7xl px-6">
          <div className="grid lg:grid-cols-[1fr_320px] gap-12 -mt-24 relative z-10">
            ...
            <article ref={articleRef}>
              ...
              <div className="bg-surface-card rounded-3xl shadow-xl border border-hairline-dark p-8 md:p-12 mb-8">
                ...
                {post.category && (
                  <Link href={`/blog/category/${post.category.slug}`}>
                    <span
                      ...
                      style={{ backgroundColor: post.category.color ?? "#b8925a" }}
                    >
                ...
                <h1 className={`text-3xl md:text-4xl lg:text-5xl font-light tracking-tight text-ink leading-tight mb-6 ${playfair.className}`}>
                  {post.title}
                </h1>

                {post.excerpt && (
                  <p className="text-lg text-ink-secondary font-light leading-relaxed mb-6 border-l-4 border-mauve/30 pl-5">
                    {post.excerpt}
                  </p>
                )}
                ...
                <div className="flex flex-wrap gap-2 mt-6">
                  {post.tags.map((tag) => (
                    <Link
                      ...
                      className="rounded-full bg-mauve/5 px-3 py-1 text-xs text-ink-secondary hover:bg-mauve/15 hover:text-mauve transition-colors"
                    >
                      #{tag.name}
                    </Link>
                  ))}
              ...
              {post.author && (
                <div className="mt-8 rounded-2xl border border-hairline-dark bg-surface-card p-6 flex gap-5">
                  ...
                  <div>
                    ...
                    <Link href={`/blog/author/${post.author.slug}`} className="font-bold text-ink hover:text-mauve transition-colors">
                      {post.author.name}
                    </Link>
                    <p className="text-xs text-ink-secondary mt-1 leading-relaxed">{post.author.bio}</p>
                  </div>
                </div>
              )}
              ...
              <div className="mt-8 rounded-2xl bg-gradient-to-br from-mauve/10 to-mauve-strong/5 border border-mauve/20 p-6">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <h3 className="font-bold text-ink text-sm">Get more guides like this</h3>
                </div>
                <p className="text-xs text-ink-secondary mb-4">Join 10,000+ event professionals getting tips, guides, and updates from Snapsy.</p>
                <InlineNewsletter />
              </div>
              ...
              <div className="mt-8 flex justify-between">
                <Link href="/blog">
                  <button className="flex items-center gap-2 rounded-full border border-hairline-dark px-5 py-2.5 text-sm text-ink-secondary hover:bg-mauve/5 hover:text-ink transition-colors">
            ...
            <aside className="mt-0 space-y-6">
              ...
              {related.length > 0 && (
                <div className="rounded-2xl border border-hairline-dark bg-surface-card p-5 sticky top-[100px]">
                  <h3 className="text-sm font-bold text-ink mb-4">You might also like</h3>
                  {related.map((r) => (
              ...
              {/* CTA */}
              <div className="rounded-2xl bg-gradient-to-br from-surface-card to-surface-card-elevated border border-hairline-dark p-6 text-ink">
          ...
          {related.length > 0 && (
            <section className="mt-16 mb-12">
              <h2 className="text-xl font-bold text-ink mb-6">Keep reading</h2>
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
                {related.map((r) => (
                  ...
                  >
                    <Link href={`/blog/${r.slug}`} className="block aspect-video overflow-hidden bg-ink/5">
                      {r.cover_image_url && (
                    ...
                    <div className="p-5">
                      {r.category && (
                        <span className="text-[10px] font-bold uppercase tracking-widest mb-2 block" style={{ color: r.category.color ?? "#b8925a" }}>
                          {r.category.emoji} {r.category.name}
                        </span>
                      )}
                      <Link href={`/blog/${r.slug}`}>
                        <h3 className="text-sm font-bold text-ink leading-snug group-hover:text-mauve transition-colors line-clamp-2">{r.title}</h3>
                      </Link>
