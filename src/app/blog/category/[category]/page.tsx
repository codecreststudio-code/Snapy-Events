      ...
      <div className="aspect-video overflow-hidden bg-ink/5">
        {post.cover_image_url ? (
          ...
          <div className="w-full h-full flex items-center justify-center">
            <BookOpen className="h-8 w-8 text-ink-tertiary" />
          </div>
        )}
      </div>
      <div className="flex-1 p-5">
        <h3 className="text-sm font-bold text-ink leading-snug mb-2 group-hover:text-mauve transition-colors line-clamp-2">{post.title}</h3>
        <p className="text-xs text-ink-secondary font-light leading-relaxed line-clamp-2 mb-4">{post.excerpt}</p>
      ...
      <main className="flex-1">
        ...
        <section className="border-b border-hairline-dark bg-surface-card py-14 md:py-20">
          <div className="container mx-auto max-w-5xl px-6 text-center">
            ...
            <div className="inline-block rounded-full px-3 py-1 text-xs font-bold text-white mb-4" style={{ backgroundColor: cat.color ?? "#b8925a" }}>
              CATEGORY
            </div>
            <h1 className={`text-4xl md:text-5xl font-light tracking-tight text-ink mb-4 ${playfair.className}`}>{cat.name}</h1>
            <p className="text-ink-secondary font-light max-w-xl mx-auto">{cat.description}</p>
            <div className="mt-4 text-xs text-ink-secondary">{posts.length} articles</div>
          </div>
        </section>
        ...
        <section className="container mx-auto max-w-5xl px-6 py-12">
          {posts.length > 0 ? (
            ...
            <div className="py-24 text-center">
              ...
              <p className="text-ink-secondary">No articles in this category yet.</p>
