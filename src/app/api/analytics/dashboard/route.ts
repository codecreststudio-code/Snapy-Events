...
export const GET = defineRoute({
  ...
  handler: async ({ query, auth }) => {
    ...
    const [{ data: events }, { data: photos }, { data: storage }] = await Promise.all([
      supabase.from("events").select("id, name, status, view_count, galleries(photo_count)").eq("host_id", auth.user!.id),
      supabase.from("photos").select("id, created_at, event:events!inner(host_id)").eq("event.host_id", auth.user!.id).gte("created_at", since),
    ...
    const totalUploads = events.reduce((s, e) => s + ((e.galleries as any[])?.reduce((sum, g) => sum + (g.photo_count || 0), 0) || 0), 0)
    return ok({
