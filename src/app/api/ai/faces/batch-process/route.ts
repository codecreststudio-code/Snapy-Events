...
export const POST = defineRoute({
  ...
  handler: async ({ body, auth }) => {
    if (!auth.organization?.feature_flags?.["ai_face_search"]) return fail("FORBIDDEN", "AI not enabled", 403)
    ...
    const result = await batchProcessFaces({ organizationId: auth.organization!.id, eventId: body.event_id, photoIds: body.photo_ids, imageUrls: urls })
    return ok(result)
  },
}).POST
