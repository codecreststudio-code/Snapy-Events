...
import { generateRecapVideo, MAX_HIGHLIGHT_PHOTOS, type RecapMood } from "@/lib/integrations/recap-video"
...
const bodySchema = z.object({
  mood: z.enum(["joyful", "sentimental"]),
  // Optional host-picked moments from the dashboard's "Select your moments"
  // picker. Omitted/empty falls back to the existing auto-selection —
  // this is purely additive, no existing caller needs to change.
  photo_ids: z.array(z.string().uuid()).max(MAX_HIGHLIGHT_PHOTOS).optional(),
})
...
export const POST = defineRoute<{ mood: RecapMood; photo_ids?: string[] }, unknown, { id: string }>({
  method: "POST",
  body: bodySchema,
  requireAuth: true,
  ...
  handler: async ({ body, params, auth }) => {
    ...
    const { mood, photo_ids: selectedPhotoIds } = body
    ...
    try {
      const result = await generateRecapVideo({ eventId, mood, supabase, selectedPhotoIds })
