      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        {[...Array(4)].map((_, i) => (
          <div key={i} className="rounded-2xl border border-[#e5dfd0] bg-[#ffffff] p-6 animate-pulse">
            <div className="h-4 w-24 rounded bg-[#e5dfd0]/60 mb-3" />
            <div className="h-8 w-16 rounded bg-[#e5dfd0]/60" />
          </div>
        ))}
      </div>
      {/* Recent events skeleton */}
      <div className="rounded-2xl border border-[#e5dfd0] bg-[#ffffff] p-6 animate-pulse space-y-4">
        <div className="h-5 w-32 rounded bg-[#e5dfd0]/60" />
        {[...Array(3)].map((_, i) => (
          <div key={i} className="flex items-center gap-4">
            <div className="h-10 w-10 rounded-full bg-[#e5dfd0]/60" />
