...
export interface GenerateRecapVideoParams {
  ...
  // Optional host-picked photo IDs from the "Select your moments" picker
  // (dashboard/events/[slug]/page.tsx). When omitted/empty, falls back to
  // the existing auto-selection (top-reacted + timeline-sampled). When
  // provided, the recap uses exactly these photos instead — the host's
  // deliberate choice always wins over the heuristic.
...
// Exported so the API route (recap/generate/route.ts) can cap the
// host-submitted `photo_ids` array to the same limit at the validation
// layer, instead of silently truncating a larger selection deep inside
// fetchSelectedPhotos() with no feedback to the host.
export const MAX_HIGHLIGHT_PHOTOS = 16
...
export async function generateRecapVideo({
  ...
  const highlightPhotos = selectedPhotoIds && selectedPhotoIds.length > 0
    ? await fetchSelectedPhotos(supabase, eventId, selectedPhotoIds)
    : await selectHighlightPhotos(supabase, eventId)
...
// Used when the host has explicitly picked moments via the "Select your
// moments" picker instead of relying on auto-selection. Scoped to this
// event's own image photos (never videos/voice notes — the montage
// compositor only knows how to Ken-Burns still images) and, like the
// auto-selected RPCs since migration 0027, not filtered by is_approved:
// approval is a guest-facing moderation concept, irrelevant to a host
// recapping their own event. IDs the host sent that don't resolve to a
// still image on this event (wrong event, deleted since the picker loaded,
// a video/voice-note id, etc.) are silently dropped rather than erroring —
// picking a still-valid subset is better than failing the whole recap.
async function fetchSelectedPhotos(
  supabase: SupabaseClient,
  eventId: string,
  photoIds: string[],
): Promise<HighlightPhoto[]> {
  const { data, error } = await supabase
    .from("photos")
    .select("id, storage_path, created_at, mime_type")
    .eq("event_id", eventId)
    .ilike("mime_type", "image/%")
    .in("id", photoIds.slice(0, MAX_HIGHLIGHT_PHOTOS))

  if (error) {
    logger.error("recap-video: selected-photos query failed", { eventId, error: error.message })
    return []
  }

  const rows = (data as HighlightPhotoRow[] | null) ?? []
  rows.sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime())
  return rows.slice(0, MAX_HIGHLIGHT_PHOTOS).map((row) => ({ id: row.id, storagePath: row.storage_path }))
}
